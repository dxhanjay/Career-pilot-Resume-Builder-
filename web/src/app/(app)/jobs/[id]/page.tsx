...
import {
  Alert, Badge, Button, Card, CardHeader, EmptyState, Field, Loading, PageHeader,
  ScoreBar, ScoreRing, Select, cx,
} from '@/components/ui'
...
export default function JobDetailPage() {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const postingData = await api.get<any>(`/job-descriptions/${id}`)
      setPosting(postingData)

      const [matchResult, resumeResult] = await Promise.allSettled([
        api.get<any>(`/job-descriptions/${id}/match`),
        api.get<any[]>('/resumes'),
      ])

      const latest = matchResult.status === 'fulfilled' ? matchResult.value : null
      setMatch(latest)
      ...
      // The resume the last match used, then the primary, then the newest — in
      // that order, because it is the order of what the user most likely means.
      setSelectedResume((current) => {
        if (current) return current
        if (latest?.resumeId && available.some((r) => r.id === latest.resumeId)) return latest.resumeId
        return available.find((r) => r.isPrimary)?.id ?? available[0]?.id ?? ''
      })
    } catch (err) {
  ...
  async function runMatch() {
    ...
    setActionError(null)
    try {
      setMatch(await api.post(`/job-descriptions/${id}/match`, { resumeId: selectedResume }))
    } catch (err) {
  ...
  return (
    <>
      <PageHeader
        breadcrumb={
          <Link href="/jobs" className="mb-2 inline-flex items-center gap-1 rounded text-sm text-ink-500 hover:text-ink-800">
        ...
        {resumes.length === 0 && (
          <div className="px-5 pb-5">
            <Alert tone="amber" title="No parsed resume yet">
              Upload a resume before matching it against a posting.{' '}
...
function MatchReport({ match }: { match: any }) {
  return (
    <div className="space-y-6">
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader
            title="How the percentage is built"
            description="Published deliberately — a score you cannot check is not a score."
          />
      ...
      {match.suggestions?.length > 0 && (
        <Card>
          ...
          <ul className="divide-y divide-ink-100">
            {match.suggestions.map((suggestion: any, index: number) => (
...
function SkillPanel({ title, description, skills = [], tone, emptyTitle, emptyDescription, showJd, showResume }: any) {
  return (
    <Card>
