...
import {
  Alert, Badge, Button, Card, CardHeader, Loading, PageHeader, ScoreBar, ScoreRing, TextArea, cx,
} from '@/components/ui'
...
export default function InterviewRoomPage() {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const data = await api.get<any>(`/interviews/${id}`)
      setSession(data)
      // Open on the first unanswered question, which is where the user left off.
      const firstUnanswered = data.questions.findIndex((question: any) => !question.answer)
  ...
  async function finish() {
    ...
    setActionError(null)
    try {
      setSession(await api.post(`/interviews/${id}/complete`))
    } catch (err) {
  ...
  return (
    <>
      <PageHeader
        breadcrumb={
          <Link href="/interviews" className="mb-2 inline-flex items-center gap-1 rounded text-sm text-ink-500 hover:text-ink-800">
      ...
      {actionError && <Alert tone="red" title="That did not work" className="mb-6">{actionError}</Alert>}
              ...
              <li key={item.id}>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {item.answer && <span className="text-xs text-ink-500">Scored {item.answer.score}</span>}
                  </span>
                </button>
              </li>
            ))}
              ...
              <div className="space-y-5 p-5">
                ...
                {isOpen && (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {current > 0 && (
                      <Button variant="secondary" onClick={() => setCurrent(current - 1)}>Previous</Button>
                    )}
                    {current < session.questions.length - 1 && (
                      <Button variant="secondary" onClick={() => setCurrent(current + 1)}>Next question</Button>
                    )}
                  </div>
                )}

                {question.answer && (
                  <AnswerFeedback answer={question.answer} expected={question.expectedPoints} />
                )}
              </div>
            </Card>
          )}
...
function AnswerFeedback({ answer, expected }: any) {
  return (
    <div className="space-y-4 rounded-xl border border-ink-200 bg-ink-50/70 p-5">
      ...
      {answer.strengths?.length > 0 && (
        <div>
          ...
          <ul className="mt-1.5 space-y-1.5">
            {answer.strengths.map((item: string) => (
              <li key={item} className="text-sm leading-6 text-ink-700">• {item}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {answer.improvements?.length > 0 && (
        <div>
          ...
          <ul className="mt-1.5 space-y-1.5">
            {answer.improvements.map((item: string) => (
              <li key={item} className="text-sm leading-6 text-ink-700">• {item}</li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="mt-1.5 space-y-1.5">
            {expected.map((item: string) => (
              <li key={item} className="text-sm leading-6 text-ink-600">• {item}</li>
            ))}
          </ul>
        </div>
      )}
  ...
  return (
    <Card className="mb-6">
      <CardHeader
        ...
        description="Unanswered questions count as zero, so this reflects the interview you actually sat."
      />
      <div className="grid gap-6 p-5 lg:grid-cols-3">
        ...
        <div className="space-y-5 lg:col-span-2">
          <div className="grid gap-5 sm:grid-cols-2">
            {report.axisScores.map((axis: any) => (
              <ScoreBar
                key={axis.axis}
          ...
          {report.topImprovements?.length > 0 && (
            <div>
              ...
              <ul className="mt-1.5 space-y-1.5">
                {report.topImprovements.map((item: string) => (
                  <li key={item} className="text-sm leading-6 text-ink-700">• {item}</li>
                ))}
              </ul>
            </div>
          ...
          {report.topStrengths?.length > 0 && (
            <div>
              ...
              <ul className="mt-1.5 space-y-1.5">
                {report.topStrengths.map((item: string) => (
                  <li key={item} className="text-sm leading-6 text-ink-700">• {item}</li>
                ))}
              </ul>
            </div>
