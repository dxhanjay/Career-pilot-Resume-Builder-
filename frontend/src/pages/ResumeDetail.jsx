 ...
 * the same `scoreTone` the rest of the page uses so the bar and the ring can
 * never disagree.
 */
const BAR_COLOURS = {
  emerald: '#10b981',
...
export default function ResumeDetail() {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const resumeData = unwrap(await api.get(`/resumes/${id}`))
      setResume(resumeData)

      // Everything below depends on a successful parse, and each is 404/409
      // until it exists. Failing the whole page because a resume has not been
      // scored yet would hide the very screen that explains why.
      const [parseResult, structuredResult, analysisResult, historyResult] = await Promise.allSettled([
        api.get(`/resumes/${id}/parse`),
        api.get(`/resumes/${id}/parse/structured`),
        api.get(`/resumes/${id}/ats`),
        api.get(`/resumes/${id}/ats/history`),
      ])

      setParse(parseResult.status === 'fulfilled' ? unwrap(parseResult.value) : null)
      setStructured(structuredResult.status === 'fulfilled' ? unwrap(structuredResult.value) : null)
      setAnalysis(analysisResult.status === 'fulfilled' ? unwrap(analysisResult.value) : null)
      setHistory(historyResult.status === 'fulfilled' ? unwrap(historyResult.value) : null)
    } catch (err) {
  ...
  // Poll while the background parse job is still running.
  useEffect(() => {
    if (!resume) return undefined
    if (resume.status !== 'UPLOADED' && resume.status !== 'PARSING') return undefined
  ...
  async function runAnalysis() {
    ...
    setActionError(null)
    try {
      setAnalysis(unwrap(await api.post(`/resumes/${id}/ats`)))
      setHistory(unwrap(await api.get(`/resumes/${id}/ats/history`)))
      setTab('score')
    } catch (err) {
  ...
  async function startParse() {
    ...
    try {
      ...
      setActionError(errorMessage(err, 'Could not start reading this resume.'))
    } finally {
      setWorking(false)
    }
  }
  ...
  const showEvidence = useCallback((lineStart) => {
  ...
  const isReading = resume.status === 'UPLOADED' || resume.status === 'PARSING'

  return (
    <>
      <PageHeader
        breadcrumb={
          <Link to="/resumes" className="mb-2 inline-flex items-center gap-1 rounded text-sm text-ink-500 hover:text-ink-800">
        ...
        action={
          <>
            ...
            {resume.status === 'PARSE_FAILED' && (
              <Button size="sm" loading={working} onClick={startParse}>
                Try reading again
              </Button>
            )}
          </>
      ...
      {isReading && (
        <Alert tone="amber" title="Reading your resume" className="mb-6">
          We are extracting the text now. This page updates itself when it is done.
        </Alert>
      )}
      ...
      {resume.status === 'PARSE_FAILED' && (
        <Alert tone="red" title="We could not read this file" className="mb-6">
          {parse?.error ??
            'No usable text could be extracted. This usually means the document is a scan or an image rather than real text.'}
        </Alert>
      )}
  ...
  return (
    {
      ...
      FALLBACK_PARSER_USED: 'A fallback reader was used',
      ENCRYPTED_DOCUMENT: 'The document is protected',
    }[code] ?? 'Parse warning'
  )
}

function ScoreTab({ analysis, working, onRun, onEvidence }) {
  const [filter, setFilter] = useState('problems')

  const findings = useMemo(() => {
    if (!analysis) return []
    if (filter === 'problems') return analysis.findings.filter((f) => f.severity !== 'PASS')
    if (filter === 'passes') return analysis.findings.filter((f) => f.severity === 'PASS')
    ...
    return (
      <Card>
        <EmptyState
          ...
          action={
            <Button size="lg" loading={working} onClick={onRun}>
              Run the analysis
            </Button>
          }
        />
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-1">
        <Card>
          <div className="flex flex-col items-center px-5 py-7">
            <ScoreRing
              ...
              tone={scoreTone(analysis.overallScore)}
              size={150}
              sublabel="/ 100"
            />
        ...
        <Card>
          <CardHeader title="Where the points went" description="Each category is weighted. Hover a name for what it means." />
          ...
          {findings.length === 0 ? (
            ...
            <ul className="divide-y divide-ink-100">
              {findings.map((finding) => (
                <FindingRow key={finding.id ?? finding.code} finding={finding} onEvidence={onEvidence} />
              ))}
            </ul>
          )}
        </Card>
...
function FindingRow({ finding, onEvidence }) {
...
function ParseTab({ parse, structured, highlight, lineRefs, onClearHighlight }) {
  const lines = structured?.lines ?? []
  const sections = useMemo(() => structured?.sections ?? [], [structured])

  // Map each line to the section it belongs to, once, rather than searching the
  // section list for every one of several hundred lines during render.
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-1">
        <Card>
          <CardHeader title="Extraction" description={`Read with ${parse.parser}`} />
          <dl className="divide-y divide-ink-100 text-sm">
            <Row label="Pages" value={parse.pageCount ?? '—'} />
            <Row label="Words" value={parse.wordCount?.toLocaleString() ?? '—'} />
            <Row label="Characters" value={parse.charCount?.toLocaleString() ?? '—'} />
            <Row label="Took" value={parse.durationMs != null ? `${parse.durationMs} ms` : '—'} />
          </dl>
        </Card>
        ...
        <Card>
          <CardHeader
            ...
            description="What a parser recognised, and how confident it is."
          />
          {sections.length === 0 ? (
            <EmptyState
...
function EntitiesTab({ structured, onEvidence }) {
  if (!structured) {
    return (
      <Card>
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        <CardHeader
          title="Contact"
          description="What a system would populate its candidate record with."
        />
        {!contact ? (
          <EmptyState
          ...
          <dl className="divide-y divide-ink-100 text-sm">
            ...
            <Row label="Portfolio" value={contact.portfolioUrl ?? <Missing />} />
          </dl>
        )}
      </Card>
      ...
      <Card>
        ...
        {skills.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2 p-5">
            {skills.map((skill) => (
              <button
                key={skill.id ?? skill.normalizedName}
                type="button"
                onClick={() => onEvidence(skill.sourceLineStart)}
                ...
                <p className="mt-1 text-xs text-ink-500">
                  {entry.startDate || entry.endDate || entry.current
                    ? `${formatDate(entry.startDate, { month: 'short', year: 'numeric' })} — ${
                        entry.current ? 'present' : formatDate(entry.endDate, { month: 'short', year: 'numeric' })
                      }`
                    : 'No dates read'}
                </p>
                {entry.sourceLineStart != null && (
                  <button
                    type="button"
                    onClick={() => onEvidence(entry.sourceLineStart)}
          ...
          <ul className="divide-y divide-ink-100">
            {education.map((entry) => (
              <li key={entry.id} className="px-5 py-4">
                ...
                <p className="mt-0.5 text-sm text-ink-600">
                  {[entry.degree, entry.fieldOfStudy].filter(Boolean).join(' · ') || <Missing label="No degree read" />}
                </p>
                <p className="mt-1 text-xs text-ink-500">
          ...
          {history.points.map((point) => (
            <div key={point.analysisId} className="flex min-w-[3.5rem] flex-col items-center gap-2">
              ...
              <div
                ...
                style={{
                  height: `${(point.overallScore / max) * 160 + 4}px`,
                  backgroundColor: BAR_COLOURS[scoreTone(point.overallScore)],
                }}
              />
              <span className="text-[10px] text-ink-400">{formatDate(point.createdAt, { day: 'numeric', month: 'short' })}</span>
            </div>
          ))}
        </div>
      </div>
...
function Missing({ label = 'Not found' }) {
