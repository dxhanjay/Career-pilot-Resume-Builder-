...
export default function Resumes() {
  ...
  const load = useCallback(async () => {
    setError(null)
    try {
      const page = unwrap(await api.get('/resumes', { params: { page: 0, size: 50 } }))
   ...
   *
   * Parsing runs as a background job, so a resume that has just been uploaded
   * shows as "Reading…" and would otherwise sit there until a manual refresh.
   * The interval clears itself once nothing is in flight, so an idle page makes
   * no requests at all.
   */
  useEffect(() => {
    const pending = resumes.some((resume) => resume.status === 'UPLOADED' || resume.status === 'PARSING')
  ...
  function validate(file) {
    ...
    const looksAccepted =
      ACCEPTED.includes(file.type) || /\.(pdf|docx)$/i.test(file.name)
    if (!looksAccepted) {
  ...
  async function upload(file) {
    ...
    // A first upload becomes the primary resume automatically; later ones do
    // not silently displace it.
    body.append('makePrimary', String(resumes.length === 0))

    try {
      const created = unwrap(
        await api.post('/resumes', body, { headers: { 'Content-Type': 'multipart/form-data' } }),
      )
      setNotice(`Uploaded ${created.originalFilename}. Reading it now — this usually takes a few seconds.`)
      await load()
    } catch (err) {
      ...
      <Card>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Once you upload a resume it appears here with the status of its parse."
          />
        ) : (
          <ul className="divide-y divide-ink-100">
            {resumes.map((resume) => {
              ...
              return (
                <li key={resume.id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2">
                    <Button as={Link} to={`/resumes/${resume.id}`} variant="secondary" size="sm">
                      Open
                    </Button>
                    {!resume.primary && (
                      <Button
